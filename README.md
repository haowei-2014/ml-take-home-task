...
Put the `candidate` folder in parallel with `solution`.
```markdown
├── candidate
│       ├── CANDIDATE_BRIEF.md
│       ├── DATA_DICTIONARY.md
...
uv run python solution/part3_transformer.py  # T0 / T1 / T2, ~35 min CPU
```
...
did not. I kept them in training. 

**ASR confidence is not monotone in score.** Deciles 1–9 rise 1.17 → 2.89, but
the lowest decile sits *above* decile 1. It is the concrete reason to prefer trees.
...
![Part 1 overview](solution/figures/part1_overview.png)
...
rows. Zero group overlap, asserted in code. 

- **MAE** (primary) — average distance from the human rating, on the product's
  own scale.
- **QWK** — ordinal agreement corrected for chance. 
...
**The held-out number is materially worse than CV** (0.522 → 0.398). 
...
- **ASR confidence** (14) — not just the mean but the shape of the per-word
  ...
  words below 0.5 and 0.6, plus two ASR-failure flags. 
...
structured set does not improve MAE. This predicts Part 3 will struggle,
...
![Part 2 models](solution/figures/part2_models.png)
...
the two swap places between CV and held-out. 
...
of the gain is the structured columns, not the text. 

![Part 3 models](solution/figures/part3_models.png)

### Engineering

**Framework.** PyTorch + HuggingFace to fine-tune. 
